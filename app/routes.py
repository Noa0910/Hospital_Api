...
# Creación de relaciones entre pacientes y responsables
...
async def upload_excel(file: UploadFile = File(...), table: str = None):
    try:
        ...
        # Procesar el archivo según la tabla seleccionada
        if table == "patients":
        ...
        cursor.close()
        connection.close()
        return JSONResponse(status_code=200, content={"message": "Datos cargados exitosamente!"})
